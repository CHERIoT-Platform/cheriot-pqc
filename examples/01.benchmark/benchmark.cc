...
int __cheri_compartment("pqc_benchmark") benchmark()
{
	...
	rc =
	  mlkem768_keypair(mlkem_public_key, mlkem_secret_key, MALLOC_CAPABILITY);
	reportStack("Generated ML-KEM key pair");
	rc = mlkem768_enc(mlkem_ciphertext,
	                  mlkem_shared_secret_original,
	                  mlkem_public_key,
	                  MALLOC_CAPABILITY);
	reportStack("Encrypted secret pair with ML-KEM");
	rc = mlkem768_dec(mlkem_shared_secret_decrypted,
	                  mlkem_ciphertext,
	                  mlkem_secret_key,
	                  MALLOC_CAPABILITY);
	...
	rc = mldsa44_keypair(mldsa_public_key, mldsa_secret_key, MALLOC_CAPABILITY);
	...
	rc =
	  mldsa44_signature(mldsa_signature,
	                    ...
	                    mldsa_secret_key,
	                    MALLOC_CAPABILITY);
	Debug::Invariant(signatureLength == MLDSASignatureBytes,
	                 "{} != {}",
	                 signatureLength,
	                 MLDSASignatureBytes);
